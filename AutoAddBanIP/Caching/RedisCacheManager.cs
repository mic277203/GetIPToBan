...
namespace AutoAddBanIP.Caching
{
    ...
    /// </summary>
    public partial class RedisCacheManager : ICacheManager, IRedisMessage
    {
        #region Fields
        ...
        public void Subscribe<T>(string channel, Action<T> handler, bool queue = true)
        {
            if (!queue)
            {
                _db.Multiplexer.PreserveAsyncOrder = false;
            }

            ISubscriber subscriber = _db.Multiplexer.GetSubscriber();

            subscriber.Subscribe(channel, delegate (RedisChannel redisChannel, RedisValue message)
            {
        ...
        public long Publish<T>(string channel, T model)
        {
            ISubscriber subscriber = _db.Multiplexer.GetSubscriber();
